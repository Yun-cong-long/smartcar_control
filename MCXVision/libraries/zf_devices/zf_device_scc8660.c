...
uint8 scc8660_set_uniform_brightness(uint16 brightness) {
    ...
    // 2. 应用更强烈的边缘补偿
    // 中心区域 (保持默认)
    return_state |= scc8660_set_reg(0x30, 0x050);  // 中心增益系数 1.0
    
    // 中间区域 (中等补偿)
    return_state |= scc8660_set_reg(0x31, 0x120);  // 增益系数 1.12
    
    // 边缘区域 (强补偿) - 增加边缘增益
    return_state |= scc8660_set_reg(0x32, 0x500);  // 增益系数 1.38 (比之前更强)
    ...
    // 4. 增加边缘专用补偿通道
    return_state |= scc8660_set_reg(0x36, 0x500);  // 额外边缘增益系数 1.25
